...
const reason = block?.reason || 'الطالب مطرود من دخول الحصة.';
  ...
  const handleAlertFreeze = async () => {
    ...
    try {
      const reason = 'قرار: طرد كامل بعد تنبيه أثناء التحضير';
      await freezeStudent({
      ...
      const st = students.find(s => s.id === ctx.studentId);
      if (st) {
    ...
    <Layout>
      {isLoading ? (
        ...
        <StudentProfileDialog
          student={profileStudentId ? students.find(s => s.id === profileStudentId) || null : null}
          group={profileStudentId ? getGroupById(students.find(s => s.id === profileStudentId)?.group_id || '') : null}
